...
function onUpdatePost(elapsed)
  ...
  for i,camName in pairs(windowList) do
    ...
    local rate = 5
    cam.x = ezLerp(0, cam.x, rate)
    cam.y = ezLerp(0, cam.y, rate)
    cam.angle = ezLerp(0, cam.angle, rate)
    cam.scale = ezLerp(1, cam.scale, rate)
  end
    -- example modchart
  ...
  if curBeat == 357 then
    ...
    setObjectCamera('grpNoteSplashes', 'out')
    setObjectCamera('ratings', 'out')
    for i = 0, 3 do
...
function ezLerp(to, from, rate)
  return lerp(to, from, boundTo(1 - (elapsed * rate), 0, 1))
end
function onBeatHit()

  if curBeat > 64 and curBeat < 128 then
    if curBeat % 1 == 0 then
      local yea = curBeat % 2 == 0
      for i,camName in pairs(windowList) do
        local cam = cams[camName]
