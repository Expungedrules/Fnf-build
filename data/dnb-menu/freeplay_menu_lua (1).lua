...
function onCreatePost()
    ...
    addSong('main', 'Main', 'dave1', '9271fd', true)
    addSong('extras', 'Extras', 'extras', '9271fd', true)
    addSong('joke-songs', 'Joke', 'joke', '9271fd', true)
    addSong('fanmade-songs', 'Secrets', 'Secrets', '9271fd', true)
    addSong('fanmade-songs1', 'Remakes-and-Remixes', 'dave1', '9271fd', true)
...
function addSong(name, displayName, icon, color, unlocked, customFont)
    ...
    makeLuaText('display'..id, dnFinal, -1, 300, (1000 * id))
...
function changeSelection(amount)
    ...
    triggerEvent('Camera Follow Pos', 430, getProperty('display'..curSelected..'.y') + (getProperty('display'..curSelected..'.x') / 90))
...
function onUpdatePost(elapsed)
    ...
    if canPressKeys then
        ...
        if not resetting then
	        if getProperty('controls.UI_LEFT_P') or getPropertyFromClass('flixel.FlxG', 'mouse.wheel') == 1 then
                holdTime = 0
	        	changeSelection(-changeAmt)
            end
	        if getProperty('controls.UI_RIGHT_P') or getPropertyFromClass('flixel.FlxG', 'mouse.wheel') == -1 then
                holdTime = 0
	        	changeSelection(changeAmt)
            end
            if getProperty('controls.UI_LEFT') then
            ...
            if getProperty('controls.UI_RIGHT') then
                local checkLastHold = math.floor((holdTime - 0.5) * 10)
	        	holdTime = holdTime + elapsed
                local checkNewHold = math.floor((holdTime - 0.5) * 10)
                if holdTime > 0.5 and checkNewHold - checkLastHold > 0 then
